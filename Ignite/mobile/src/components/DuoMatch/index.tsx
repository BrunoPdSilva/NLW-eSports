...
export function DuoMatch({ discord, onClose, ...rest }: Props) {
  ...
  async function handleCopyDiscordToClipboard() {
    ...
    Alert.alert('Discord Copiado!', 'Usuário copiado para sua a área de transferência') 
    setIsCopping(false);
  }
      ...
      <View style={styles.container}>
        <View style={styles.content}>
          ...
          <TouchableOpacity style={styles.discordButton} onPress={handleCopyDiscordToClipboard} disabled={isCopping}>
            <Text style={styles.discord}>{isCopping ? <Activity color={THEME.COLORS.PRIMARY} /> : discord}</Text>
          </TouchableOpacity>
